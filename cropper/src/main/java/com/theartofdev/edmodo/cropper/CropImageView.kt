...
class CropImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) : FrameLayout(context, attrs) {
    ...
    var cropShape: CropShape?
        get() = mCropOverlayView.getCropShape()
        set(cropShape) {
    ...
    var guidelines: Guidelines?
        get() = mCropOverlayView.getGuidelines()
        set(guidelines) {
    ...
    val aspectRatio: Pair<Int, Int>
        get() = Pair(mCropOverlayView.getAspectRatioX(), mCropOverlayView.getAspectRatioY())
    ...
    var cropRect: Rect?
        get() {
            ...
            return BitmapUtils.getRectFromPoints(points, orgWidth, orgHeight,
                    mCropOverlayView!!.isFixAspectRatio, mCropOverlayView.aspectRatioX, mCropOverlayView.aspectRatioY)
        }
        set(rect) {
    ...
    fun getCroppedImage(reqWidth: Int, reqHeight: Int, options: RequestSizeOptions): Bitmap? {
        ...
        if (mBitmap != null) {
            ...
            croppedBitmap = if (imageUri != null && (mLoadedSampleSize > 1 || options == RequestSizeOptions.SAMPLING)) {
                val orgWidth = mBitmap!!.width * mLoadedSampleSize
                val orgHeight = mBitmap!!.height * mLoadedSampleSize
                val bitmapSampled = BitmapUtils.cropBitmap(context, imageUri!!, cropPoints,
                        mDegreesRotated, orgWidth, orgHeight,
                        mCropOverlayView!!.isFixAspectRatio, mCropOverlayView.aspectRatioX, mCropOverlayView.aspectRatioY,
                        reqWidth, reqHeight, mFlipHorizontally, mFlipVertically)
                bitmapSampled!!.bitmap
            } else {
                BitmapUtils.cropBitmapObjectHandleOOM(mBitmap!!, cropPoints, mDegreesRotated,
                        mCropOverlayView!!.isFixAspectRatio, mCropOverlayView.aspectRatioX, mCropOverlayView.aspectRatioY,
     ...
     * @param reqHeight the height to resize the cropped image to
     */
    fun saveCroppedImageAsync(saveUri: Uri?, saveCompressFormat: CompressFormat?, saveCompressQuality: Int, reqWidth: Int, reqHeight: Int) {
     ...
     */
    @JvmOverloads
    fun saveCroppedImageAsync(saveUri: Uri?, saveCompressFormat: CompressFormat? = CompressFormat.JPEG, saveCompressQuality: Int = 90, reqWidth: Int = 0, reqHeight: Int = 0, options: RequestSizeOptions? = RequestSizeOptions.NONE) {
        requireNotNull(mOnCropImageCompleteListener) { "mOnCropImageCompleteListener is not set" }
     ...
     * Can be used with URI from adapter or camera source.<br></br>
    ...
    fun rotateImage(degrees: Int) {
        ...
        if (mBitmap != null) {
            ...
            mZoom /= Math.sqrt(Math.pow((BitmapUtils.POINTS2[4] - BitmapUtils.POINTS2[2]).toDouble(), 2.0) + Math.pow((BitmapUtils.POINTS2[5] - BitmapUtils.POINTS2[3]).toDouble(), 2.0)).toFloat()
            mZoom = Math.max(mZoom, 1f)
            applyImageMatrix(width.toFloat(), height.toFloat(), true, false)
            mImageMatrix.mapPoints(BitmapUtils.POINTS2, BitmapUtils.POINTS)

            // adjust the width/height by the changes in scaling to the image
            val change = Math.sqrt(Math.pow((BitmapUtils.POINTS2[4] - BitmapUtils.POINTS2[2]).toDouble(), 2.0) + Math.pow((BitmapUtils.POINTS2[5] - BitmapUtils.POINTS2[3]).toDouble(), 2.0))
    ...
    fun onImageCroppingAsyncComplete(result: BitmapCroppingWorkerTask.Result) {
        ...
        if (listener != null) {
            val cropResult = CropResult(mBitmap, imageUri, result.bitmap, result.uri, result.error,
                    cropPoints, cropRect, wholeImageRect, rotatedDegrees, result.sampleSize)
     ...
     */
    fun startCropWorkerTask(reqWidth: Int, reqHeight: Int, options: RequestSizeOptions?, saveUri: Uri?, saveCompressFormat: CompressFormat?, saveCompressQuality: Int) {
        var reqWidth = reqWidth
        var reqHeight = reqHeight
        val bitmap = mBitmap
        if (bitmap != null) {
            ...
            mBitmapCroppingWorkerTask = if (imageUri != null && (mLoadedSampleSize > 1 || options == RequestSizeOptions.SAMPLING)) {
                WeakReference(BitmapCroppingWorkerTask(this, imageUri, cropPoints,
                        mDegreesRotated, orgWidth, orgHeight,
                        mCropOverlayView!!.isFixAspectRatio, mCropOverlayView.aspectRatioX, mCropOverlayView.aspectRatioY,
                        reqWidth, reqHeight, mFlipHorizontally, mFlipVertically, options!!,
                        saveUri, saveCompressFormat!!, saveCompressQuality))
            } else {
                WeakReference(BitmapCroppingWorkerTask(this, bitmap, cropPoints, mDegreesRotated,
                        mCropOverlayView!!.isFixAspectRatio, mCropOverlayView.aspectRatioX, mCropOverlayView.aspectRatioY,
                        reqWidth, reqHeight, mFlipHorizontally, mFlipVertically, options!!,
                        saveUri, saveCompressFormat!!, saveCompressQuality))
            }
    ...
    public override fun onSaveInstanceState(): Parcelable? {
        ...
        if (isSaveBitmapToInstanceState && imageUri == null && mImageResource < 1) {
            imageUri = BitmapUtils.writeTempStateStoreBitmap(context, mBitmap, mSaveInstanceStateBitmapUri)
        ...
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            if (inProgress) {
                ...
                if (mZoom < mMaxZoom && cropRect!!.width() < width * 0.5f && cropRect!!.height() < height * 0.5f) {
                    newZoom = Math.min(mMaxZoom.toFloat(), Math.min(width / (cropRect!!.width() / mZoom / 0.64f), height / (cropRect!!.height() / mZoom / 0.64f)))
                }
                if (mZoom > 1 && (cropRect!!.width() > width * 0.65f || cropRect!!.height() > height * 0.65f)) {
                    newZoom = Math.max(1f, Math.min(width / (cropRect!!.width() / mZoom / 0.51f), height / (cropRect!!.height() / mZoom / 0.51f)))
                }
                if (!mAutoZoomEnabled) {
                    newZoom = 1f
                }
    ...
    private fun applyImageMatrix(width: Float, height: Float, center: Boolean, animate: Boolean) {
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            if (mDegreesRotated > 0) {
                mImageMatrix.postRotate(mDegreesRotated.toFloat(), BitmapUtils.getRectCenterX(mImagePoints), BitmapUtils.getRectCenterY(mImagePoints))
                mapImagePointsByImageMatrix()
            }

            // scale the image to the image view, image rect transformed to know new width/height
            val scale = Math.min(width / BitmapUtils.getRectWidth(mImagePoints), height / BitmapUtils.getRectHeight(mImagePoints))
            if (scaleType == ScaleType.FIT_CENTER || scaleType == ScaleType.CENTER_INSIDE && scale < 1 || scale > 1 && mAutoZoomEnabled) {
            ...
            if (center) {
                ...
                mZoomOffsetX = if (width > BitmapUtils.getRectWidth(mImagePoints)) 0 else Math.max(Math.min(width / 2 - cropRect!!.centerX(), -BitmapUtils.getRectLeft(mImagePoints)), getWidth() - BitmapUtils.getRectRight(mImagePoints)) / scaleX
                mZoomOffsetY = if (height > BitmapUtils.getRectHeight(mImagePoints)) 0 else Math.max(Math.min(height / 2 - cropRect!!.centerY(), -BitmapUtils.getRectTop(mImagePoints)), getHeight() - BitmapUtils.getRectBottom(mImagePoints)) / scaleY
            } else {
    ...
    private fun updateImageBounds(clear: Boolean) {
        if (mBitmap != null && !clear) {
            ...
            val scaleFactorWidth = mBitmap!!.width * mLoadedSampleSize / BitmapUtils.getRectWidth(mImagePoints)
            val scaleFactorHeight = mBitmap!!.height * mLoadedSampleSize / BitmapUtils.getRectHeight(mImagePoints)
            mCropOverlayView!!.setCropWindowLimits(width.toFloat(), height.toFloat(), scaleFactorWidth, scaleFactorHeight)
        }
        ...
        if (options == null) {
            ...
            if (attrs != null) {
                ...
                try {
                    ...
                    isSaveBitmapToInstanceState = ta.getBoolean(R.styleable.CropImageView_cropSaveBitmapToInstanceState, isSaveBitmapToInstanceState)

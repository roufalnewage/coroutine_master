...
@JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    private fun drawBackground(canvas: Canvas) {
        ...
        if (mCropShape == CropShape.RECTANGLE) {
            ...
            mPath.reset()
            if (Build.VERSION.SDK_INT >= 11 && Build.VERSION.SDK_INT <= 17 && mCropShape == CropShape.OVAL) {
    ...
    private fun drawCorners(canvas: Canvas) {
        if (mBorderCornerPaint != null) {
            ...
            val w: Float = (cornerWidth / 2) + (if (mCropShape == CropShape.RECTANGLE) mBorderCornerOffset else 0).toFloat()
            val rect = mCropWindowHandler.rect
            rect!!.inset(w, w)
